...
- [type Logger](<#Logger>)
  ...
  - [func \(l \*Logger\) SetFileOnly\(fileOnly bool\)](<#Logger.SetFileOnly>)
...
<a name="Logger.SetFileOnly"></a>
### func \(\*Logger\) SetFileOnly

```go
func (l *Logger) SetFileOnly(fileOnly bool)
```

SetFileOnly dynamically switches between file\-only and stdout\+file output modes. When fileOnly is true, logs only go to file; when false, logs go to both stdout and file.
