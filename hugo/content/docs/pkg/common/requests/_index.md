...
- [type Client](<#Client>)
  ...
  - [func \(c \*Client\) GetHTTPClient\(\) \*http.Client](<#Client.GetHTTPClient>)
  - [func \(c \*Client\) GetHeaders\(\) Headers](<#Client.GetHeaders>)
...
GetHTTPClient returns the underlying http.Client This is useful when creating new requests.Client instances that share the same underlying transport \(e.g., for OAuth token refresh\)

<a name="Client.GetHeaders"></a>
